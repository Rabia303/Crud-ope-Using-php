...
include('conn.php');
$uid = $_GET['id'];
$show = "SELECT * FROM `form` WHERE ID = $uid";
$r = mysqli_query($conn,$show);
...
if(isset($_POST['sub'])){
    ...
    $sql = "UPDATE `form` SET `USER_NAME`='$name',`EMAIL`='$email',`PASSWORD`='$pass',`AGE`='$age',`PHONE`='$num' WHERE ID = $uid";
    ...
    <div class="container">
      <form  class="form" method = "post">
        ...
        <div class="form-control">
          ...
          <input type="text" id="username" placeholder="Enter username" name="uname" value="<?php echo $fetch['USER_NAME'] ?>" />
          <small>Error message</small>
        </div>
        <div class="form-control">
          <label for="email">Email</label>
          <input type="email" id="email" placeholder="Enter email" name="uemail"value="<?php echo $fetch['EMAIL'] ?>" />
          <small>Error message</small>
        </div>
        <div class="form-control">
          <label for="password">Password</label>
          <input type="password" id="password" placeholder="Enter password"  name="upass" value="<?php echo $fetch['PASSWORD'] ?>" />
          <small>Error message</small>
        </div>
        <div class="form-control">
          <label for="age" >Age</label>
          <input type="age" id="password2" placeholder="Enter Your Age" name="uage" value="<?php echo $fetch['AGE'] ?>" />
          <small>Error message</small>
        </div>

         <div class="form-control">
          <label for="number">Phone</label>
          <input type="Phnoe" id="password2" placeholder="Enter Your Phone Number" name="unum" value="<?php echo $fetch['PHONE'] ?>" />
          <small>Error message</small>
        </div>
        <button type="submit" value="Update" name="sub">Register</button>
      </form>
    </div>
